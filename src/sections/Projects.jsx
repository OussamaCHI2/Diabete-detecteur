  ...
  const handleSubmitModelData = async (e) => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:5000/predict', {
        ...
        body: JSON.stringify({...formData, model_id: 1 }), // Include selected model ID
      });

      const result = await response.json();
      
      setPredictionResult(result.prediction);
     // Raise The Result popup
          ...
          <div className={`p-6 rounded-lg shadow-lg ${prediction === 1 ? 'bg-gradient-to-br from-red-900 to-red-800' : 'bg-gradient-to-br from-green-900 to-green-800'}`}>
            ...
            <div className="bg-black bg-opacity-20 rounded-lg p-4 mb-4">
              <div className="flex justify-between items-center">
                <span className="text-gray-300">Prédiction Resultat:</span>
            ...
            <div className="mt-4">
              ...
              <div className="flex justify-between mt-1">
                <span className="text-xs text-green-300">Bas Risk</span>
                <span className="text-xs text-red-300">Haut Risk</span>
              </div>
            </div>
            
            {prediction === 1 ? (
              <div className="mt-4 p-3 bg-red-950 bg-opacity-50 rounded border border-red-700 text-sm text-white">
                <p>Envisagez des tests supplémentaires et une consultation avec un professionnel de la santé pour un diagnostic et des options de gestion appropriés.</p>
              </div>
            ) : (
              <div className="mt-4 p-3 bg-green-950 bg-opacity-50 rounded border border-green-700 text-sm text-white">
                <p>Les résultats suggèrent un faible risque de diabète. Maintenir de saines habitudes de vie pour une prévention continue.</p>
              </div>
            )}
          </div>
        </div>
  ...
  <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity duration-300 p-4 animate-fade-in">
    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-2xl w-full max-w-3xl border border-blue-100 dark:border-blue-900 max-h-[90vh] overflow-y-auto animate-scale-up">
      ...
      <p className="text-gray-600 dark:text-gray-300 mb-4 text-sm animate-fade-in-up delay-100">
        Veuillez saisir vos paramètres de santé ci-dessous pour générer une évaluation du risque de diabète.
      </p>
...
Nombre de fois enceinte</p>
            ...
            <div className="mb-3">
              ...
              <input 
                ...
                required 
              />
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Concentration de glucose plasmatique</p>
            </div>
            
            <div className="mb-3">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 flex items-center">
                <span className="mr-1">❤️</span> Pression artérielle
              </label>
              <input 
                type="number" 
                name="BloodPressure" 
                ...
                required 
              />
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Pression artérielle diastolique</p>
            </div>
...
Épaisseur de la peau
              ...
              />
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
Épaisseur du pli cutané du triceps</p>
            </div>
          </div>
          ...
          <div className="animate-fade-in-right delay-300">
            <div className="mb-3">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 flex items-center">
                <span className="mr-1">💉</span> Insuline
              </label>
              <input 
                type="number" 
                name="Insulin" 
                ...
                onChange={handleInputChange} 
                placeholder="Example: 16-846 μU/ml" 
                className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white text-sm transform transition-all duration-300 hover:shadow-md" 
                required 
              />
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
Insuline sérique sur 2 heures</p>
            </div>
            
            <div className="mb-3">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 flex items-center">
                <span className="mr-1">⚖️</span> IMC
              </label>
              <input 
                type="number" 
                name="BMI" 
...
Indice de masse corporelle</p>
            ...
            <div className="mb-3">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 flex items-center">
                <span className="mr-1">👪</span> Antécédents familiaux
              </label>
              <input 
                type="number" 
              ...
              />
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
Score d'influence génétique</p>
            </div>
            
            <div className="mb-3">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 flex items-center">
                <span className="mr-1">📆</span> Âge
              </label>
              <input 
                type="number" 
                name="Age" 
        ...
        <div className="mt-6 pt-3 border-t border-gray-200 dark:border-gray-700 animate-fade-in-up delay-400">
          <div className="flex items-center mb-4">
            ...
            <p className="ml-2 text-xs text-gray-500 dark:text-gray-400">
              Vos données de santé sont traitées de manière sécurisée et confidentielle
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-2">
            ...
            >
              <span className="mr-2 group-hover:mr-3 transition-all duration-300">Prédire</span>
    ...
    <DiabetesPredictionDisplay 
     ...
     />
          ...
          <div className="flex justify-between items-center mt-7">
            <button className="arrow-btn transition duration-300 ease-in-out transform hover:scale-105 hover:bg-red-800 " onClick={() => handleNavigation('previous')}>
              <img src="/assets/left-arrow.png" alt="left arrow" />
            </button>
  {/* Button de Choisir le model et Affiche le pop up*/}
              <button 
  className="flex items-center gap-2 px-4 py-2 bg-orange-500 hover:bg-red-800 text-black hover:text-white font-semibold rounded-xl shadow-md transition duration-300 ease-in-out transform hover:scale-105"
  onClick={() => { handleModelSelection(currentProject.id); setShowPopup(true); }}
>
  <img src="/assets/arrow-up.png" alt="arrow" className="w-4 h-4" />
  <p>Tester Votre Risque du Diabète</p>
</button>


            <button className="arrow-btn transition duration-300 ease-in-out transform hover:scale-105 hover:bg-cyan-800" onClick={() => handleNavigation('next')}>
